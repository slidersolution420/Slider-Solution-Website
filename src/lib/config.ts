...
const DEFAULTS: SiteConfig = {
  ...
  ticker_es: '✦ Envío Gratis a Todo el País ✦ El Kit Slider Original ✦ +1,000 Clientes Satisfechos ✦',
  ticker_de: '✦ Kostenloser Versand in der EU ✦ Das Original Slider Kit ✦ 1.000+ zufriedene Kunden ✦',
...
export async function getConfig(): Promise<SiteConfig> {
  ...
  try {
    ...
    configCache = {
      ...
      ticker_es:               (map.ticker_es               as string)    ?? DEFAULTS.ticker_es,
      ticker_de:               (map.ticker_de               as string)    ?? DEFAULTS.ticker_de,
